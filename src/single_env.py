...
class SingleNBIoT(gym.Env):
   ...
   def _action_to_params(self, action):
      params = [12, 24, 36, 48] # Preamble Frequency
      preamble_freq = params[action]
      return preamble_freq, 4, 1
   ...
   def step(self, action: int):
      ...
      self.obs = np.array(no_of_successes)
      self.tti += 1

      return self._get_obs(), no_of_successes/10, self.tti == len(TTIS), self.tti == len(TTIS), {}